...
export const GTFS_PRIMARY_KEYS: GTFSTablePrimaryKey[] = [
  ...
  },
  {
    ...
    fields: ['network_id', 'from_area_id', 'to_area_id', 'from_timeframe_group_id', 'to_timeframe_group_id', 'fare_product_id'],
    type: 'composite'
  },
  {
 ...
 */
export function getGTFSPrimaryKey(tableName: string): GTFSTablePrimaryKey | null {
  return GTFS_PRIMARY_KEYS.find(config => config.tableName === tableName) || null;
}
 ...
 */
export function generateCompositeKeyFromRecord(tableName: string, record: Record<string, any>): string {
  const config = getGTFSPrimaryKey(tableName);
  ...
  if (config.type === 'natural') {
    ...
    if (value === undefined || value === null || value === '') {
      throw new Error(`Missing required primary key field '${field}' for table '${tableName}'`);
    }
    return String(value);
  }
  ...
  throw new Error(`Unsupported primary key type '${config.type}' for table '${tableName}'`);
}