...
import { GTFSRecord, Agency, Stops, Routes, Trips } from '../types/gtfs.js';
...
export class GTFSRelationshipResolver {
  ...
  public getReferencingRecords(
    ...
    return Array.from(sourceMap.values()).filter(record => 
      (record as any)[sourceKeyField] === targetKeyValue
    );
  }
  ...
  public enhanceStop(stopId: string): EnhancedStop | undefined {
    ...
    if (parentStationId) {
      enhanced.parentStationDetails = this.database['stops.txt']?.get(parentStationId) as Stops;
    }
    
    // Get child stops (if this is a station)
    enhanced.childStops = this.getReferencingRecords(
      stopId, 'stops.txt', 'parentStation'
    ) as Stops[];
  ...
  public enhanceRoute(routeId: string): EnhancedRoute | undefined {
    ...
    for (const trip of enhanced.trips || []) {
      const tripStopTimes = this.getReferencingRecords(
        (trip as any).tripId, 'stop_times.txt', 'tripId'
      );
  ...
  public enhanceTrip(tripId: string): EnhancedTrip | undefined {
    ...
    if (serviceId) {
      enhanced.servicePattern = this.database['calendar.txt']?.get(serviceId) ||
                               this.database['calendar_dates.txt']?.get(serviceId);
    }
    
    // Get stop times
    enhanced.stopTimes = this.getReferencingRecords(
      tripId, 'stop_times.txt', 'tripId'
    ).sort((a, b) => ((a as any).stopSequence || 0) - ((b as any).stopSequence || 0));
    
    // Get shape
    const shapeId = (trip as any).shapeId;
    if (shapeId) {
      enhanced.shape = this.getReferencingRecords(
        shapeId, 'shapes.txt', 'shapeId'
      ).sort((a, b) => ((a as any).shapePtSequence || 0) - ((b as any).shapePtSequence || 0));
    }
    
    // Get frequencies
    enhanced.frequencies = this.getReferencingRecords(
      tripId, 'frequencies.txt', 'tripId'
    );
  ...
  public getAvailableRelationships(filename: string): GTFSRelationship[] {
    return GTFS_RELATIONSHIPS.filter(rel => 
      rel.sourceFile === filename || rel.targetFile === filename
    );
  }
}